...
If you need explorer-friendly historical state retention for a longer debug run, use `start-all-archive.sh` instead.
...
| `start-all-archive.sh` | Runs the same full relay-backed stack as `start-all.sh`, but with the local collator in archive mode. | Use this when you want to inspect older local blocks, decode historical runtime events in the explorer, or keep longer scan/debug windows during a run. |
...
| `start-local-archive.sh` | Runs the same relay-backed network as `start-local.sh`, but with archive pruning disabled for the collator. | Use this when you only need the network itself and still want historical state available for explorer or runtime-event queries. |
| `start-relayer.sh` | Installs relayer dependencies and starts the local StealthPay private-withdraw relayer on `127.0.0.1:8787` by default. | Use this when testing the direct sender-to-pool private withdraw flow locally or when you want a simple relayer for the judge-facing demo. |
| `deploy-paseo.sh` | Installs dependencies, compiles, and deploys the EVM and PVM contracts to the Polkadot testnet configuration used by the Hardhat projects. | Use this when you are deploying contract examples to testnet rather than running them locally. Set the repo-root `.env` first so `PRIVATE_KEY` is available to both contract packages. |
...
- `start-all-archive.sh` and `start-local-archive.sh` are thin wrappers that set `STACK_ARCHIVE_MODE=1` before delegating to the existing startup scripts.
- `start-relayer.sh` defaults to Alice's public dev key only for the local chain. For Paseo or any non-local target, set `RELAYER_PRIVATE_KEY` in the repo-root `.env` first.
- The Hardhat configs in `contracts/evm` and `contracts/pvm` now load the repo-root `.env` automatically. `.env.example` shows the supported keys.
- Archive mode only preserves history for runs started in archive mode. It does not restore state that was already pruned in an earlier run.
