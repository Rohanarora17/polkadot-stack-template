...
interface INativePoseidonHasher2 {
	function poseidon(uint256[2] memory input) external pure returns (uint256);
}

/// @title StealthPayPoolNativeV1
/// @notice Experimental native-denomination deposit pool for testing Substrate-origin Revive.call.
/// @dev This is a deposit-path probe, not the production private-withdraw pool.
contract StealthPayPoolNativeV1 {
	uint8 public constant TREE_DEPTH = 10;
	uint256 public constant MAX_LEAVES = 1 << TREE_DEPTH;
	uint256 public constant DENOMINATION = 1_000_000_000_000;
	uint256 private constant SNARK_SCALAR_FIELD =
		21888242871839275222246405745257275088548364400416034343698204186575808495617;

	INativePoseidonHasher2 public immutable hasher;
	...
	constructor(address poseidonHasher) {
		hasher = INativePoseidonHasher2(poseidonHasher);
