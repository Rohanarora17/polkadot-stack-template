...
import { getAddress, parseEventLogs, keccak256, toBytes } from "viem";

describe("ProofOfExistence (EVM)", function () {
	async function deployFixture() {
		const [owner, otherAccount] = await hre.viem.getWalletClients();
		const poe = await hre.viem.deployContract("ProofOfExistence");
		const publicClient = await hre.viem.getPublicClient();
		return { poe, owner, otherAccount, publicClient };
	}
	...
	it("Should emit ClaimCreated event", async function () {
		const { poe, owner, publicClient } = await loadFixture(deployFixture);
		const hash = await poe.write.createClaim([testHash]);
	...
	it("Should emit ClaimRevoked event", async function () {
		const { poe, owner, publicClient } = await loadFixture(deployFixture);
		await poe.write.createClaim([testHash]);
		const hash = await poe.write.revokeClaim([testHash]);
		const receipt = await publicClient.waitForTransactionReceipt({ hash });
		const logs = parseEventLogs({
			abi: poe.abi,
			logs: receipt.logs,
