...
const INDEXER_WARMUP_TIMEOUT_MS = Number(process.env.STEALTHPAY_INDEXER_WARMUP_TIMEOUT_MS || 25_000);
...
function isAllowedCorsOrigin(origin) {
	...
	try {
		...
		return (
			...
			hostname === "web-rouge-one-36.vercel.app" ||
...
app.get("/warmup", async (_req, res) => {
	...
	const indexer = await refreshIndexerForRead({
		reason: "warmup",
		timeoutMs: INDEXER_WARMUP_TIMEOUT_MS,
	});
	res.json({
		indexer,
		ok: true,
		uptimeSeconds: Math.floor(process.uptime()),
		warmedInMs: Date.now() - startedAt,
	});
});
...
app.get("/index/status", async (_req, res) => {
	const finalizedBlock =
		INDEXER_ENABLED && indexerContracts.registryAddress && indexerContracts.poolAddress
			? await getIndexerFinalizedBlock().catch(() => null)
			: null;
	const latestIndexed =
	...
	res.json({
		...
		lagBlocks:
			finalizedBlock !== null && latestIndexed !== null
				? (finalizedBlock - latestIndexed).toString()
				: null,
...
app.get("/index/deposit", async (req, res) => {
	...
	let deposit = findDeposit({ commitment, pool });
	let refreshed = null;
	if (!deposit) {
		refreshed = await refreshIndexerForRead({
			reason: "deposit-miss",
			timeoutMs: INDEXER_READ_REFRESH_TIMEOUT_MS,
		});
		deposit = findDeposit({ commitment, pool });
	}
	res.json({ deposit, ok: true, refreshed });
});

app.get("/index/deposits", async (req, res) => {
	const pool = normalizeAddress(req.query.pool);
	if (!pool) {
		res.status(400).json({ error: "pool is required" });
		return;
	}
	const refreshed = await refreshIndexerForRead({
		reason: "deposits-list",
		timeoutMs: INDEXER_READ_REFRESH_TIMEOUT_MS,
	});
	...
	const deposits = indexerState.deposits
		...
		.slice(0, limit);
	res.json({ deposits, ok: true, refreshed });
});

app.get("/index/announcement", async (req, res) => {
	...
	let announcement = findAnnouncement({ memo, registry });
	let refreshed = null;
	if (!announcement) {
		refreshed = await refreshIndexerForRead({
			reason: "announcement-miss",
			timeoutMs: INDEXER_READ_REFRESH_TIMEOUT_MS,
		});
		announcement = findAnnouncement({ memo, registry });
	}
	res.json({ announcement, ok: true, refreshed });
});

app.get("/index/withdrawal", async (req, res) => {
	const pool = normalizeAddress(req.query.pool);
	...
	let withdrawal = findWithdrawal({ nullifierHash, pool });
	let refreshed = null;
	if (!withdrawal) {
		refreshed = await refreshIndexerForRead({
			reason: "withdrawal-miss",
			timeoutMs: INDEXER_READ_REFRESH_TIMEOUT_MS,
		});
		withdrawal = findWithdrawal({ nullifierHash, pool });
	}
	res.json({ ok: true, refreshed, withdrawal });
});
...
async function runIndexerCycle() {
	if (indexerCyclePromise) {
		return indexerCyclePromise;
	}
	indexerCyclePromise = executeIndexerCycle().finally(() => {
		indexerCyclePromise = null;
	});
	return indexerCyclePromise;
}

async function executeIndexerCycle() {
	indexerRunning = true;
	try {
...
async function refreshIndexerForRead({ reason, timeoutMs }) {
	if (!INDEXER_ENABLED) {
		return { attempted: false, reason, status: "disabled" };
	}
	if (!indexerContracts.registryAddress || !indexerContracts.poolAddress) {
		return { attempted: false, reason, status: "missing-contracts" };
	}
	...
	const result = await withTimeout(runIndexerCycle(), timeoutMs, {
		attempted: true,
		reason,
		status: "timeout",
		timeoutMs,
	...
	return {
		...
		latestIndexedBlock: indexerState.latestIndexedBlock,
		reason,
		status: indexerLastError ? "error" : "ok",
	};
}
...
function findDeposit({ commitment, pool }) {
	return (
...
function findAnnouncement({ memo, registry }) {
	return (
...
function findWithdrawal({ nullifierHash, pool }) {
	return (
